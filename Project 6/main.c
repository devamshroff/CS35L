        ...
        for( int py=0; py<height; ++py )
        {
         const double y = pixel_dy * ((double)( py-(height/2) ));
	       ...
	       for( int ys=-halfSamples; ys<=halfSamples; ++ys )
            ...
            float temp[3];
            temp[0] = gamma( pixel_color[0] ) * max_color;
            temp[1] = gamma( pixel_color[1] ) * max_color;
            temp[2] = gamma( pixel_color[2] ) * max_color;

            /* enforce caps, replace with real gamma */
            for( int i=0; i<3; i++)
                temp[i] = max( min(temp[i], 255), 0);
...
main( int argc, char **argv )
{
    ...
    for(thread_id = 0; thread_id < nthreads; ++thread_id){
      ...
      fail = pthread_create(&threadList[thread_id], NULL, multiFunc, &thread_ids[thread_id]);
      if(fail){
	fprintf(stderr, "ERROR: Unsuccessful Thread Creation");
	exit(1);
      }
    }
        /* 3rd argument is the function to be multithreaded*/
	/*4th argument is the argument to that function*/		    

    for( thread_id = 0; thread_id<nthreads; thread_id++){
      fail = pthread_join(threadList[thread_id], NULL);
      if(fail){
	fprintf(stderr,"ERROR: Thread failed to finish");
	exit(1);
      }
    }
